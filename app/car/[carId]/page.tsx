...
import { getCarAppDeepLink, getCarLink, getCarPath, safeDecodeParam } from "@/app/lib/links";
...
export default async function CarLinkFallbackPage({ params }: PageProps) {
  ...
  const carId = safeDecodeParam(carIdParam);
  const publicUrl = getCarLink(carId);
  const path = getCarPath(carId);
  const appDeepLink = getCarAppDeepLink(carId);
